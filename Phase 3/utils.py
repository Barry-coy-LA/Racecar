...
def visualize(robot, particles, best_particle, radar_list, step, title, output_path, visualize=False):
    ...
    for (x, y) in radar_list:
        ax2.plot(x, y, "yo", markersize=0.1)

    # draw tragectory
    true_path = np.array(robot.trajectory)
    ax2.plot(true_path[:, 0], true_path[:, 1], "b",markersize=0.1)
    estimated_path = np.array(best_particle.trajectory)
    ax1.plot(estimated_path[:, 0], estimated_path[:, 1], "g",markersize=0.1)

    # draw particles position
    for p in particles:
        ax1.plot(p.x, p.y, "go", markersize=0.1)

    # draw robot position
    ax2.plot(robot.x, robot.y, "bo", markersize=0.1)
